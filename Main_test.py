...
def main(GUI, chosen_difficulty):
    case_instance = Case(chosen_difficulty)                                                 # Pass chosen difficulty level to Case instance
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1 and not timer_started:                                 # Left click and timer not started
                    if GUI.x >= 0 and GUI.x < GUI.x_max and GUI.y >= 0 and GUI.y < GUI.y_max:
                        ...
                        bomb_count = case_instance.count_bombs()   # I added Count bombs 
                        print("Nombre de bombes :", bomb_count)    # I added Count bombs
                        GUI.running = True
                    ...
                    if GUI.x >= 0 and GUI.x < GUI.x_max and GUI.y >= 0 and GUI.y < GUI.y_max:
                        case_instance.clique()
                        if case_instance.is_bomb_clicked(GUI.x, GUI.y):  # i
